...
module tb;
  ...
  reg push_btn;  // BTN5 ????????????????°§???????? 1
  reg reset_btn;  // BTN6 ??¶À?????????????°§????????? 1

  reg [3:0] touch_btn;  // BTN1~BTN4???????????????? 1
  reg [31:0] dip_sw;  // 32 ¶À?????????????ON????? 1
  ...
  wire [31:0] base_ram_data;  // BaseRAM ??????? 8 ¶À?? CPLD ?????????????
  wire [19:0] base_ram_addr;  // BaseRAM ???
  wire[3:0] base_ram_be_n;    // BaseRAM ???????????ßπ??????????????????????? 0
  wire base_ram_ce_n;  // BaseRAM ????????????
  wire base_ram_oe_n;  // BaseRAM ????????????
  wire base_ram_we_n;  // BaseRAM ß’??????????

  wire [31:0] ext_ram_data;  // ExtRAM ????
  wire [19:0] ext_ram_addr;  // ExtRAM ???
  wire[3:0] ext_ram_be_n;    // ExtRAM ???????????ßπ??????????????????????? 0
  wire ext_ram_ce_n;  // ExtRAM ????????????
  wire ext_ram_oe_n;  // ExtRAM ????????????
  wire ext_ram_we_n;  // ExtRAM ß’??????????

  wire [22:0] flash_a;  // Flash ?????a0 ???? 8bit ????ßπ??16bit ????????
  wire [15:0] flash_d;  // Flash ????
  wire flash_rp_n;  // Flash ??¶À????????ßπ
  wire flash_vpen;  // Flash ß’??????????????????????????
  wire flash_ce_n;  // Flash ??????????????
  wire flash_oe_n;  // Flash ???????????????
  wire flash_we_n;  // Flash ß’?????????????
  wire flash_byte_n; // Flash 8bit ????????ßπ??????? flash ?? 16 ¶À????????? 1
  ...
  parameter BASE_RAM_INIT_FILE = "D:\\github\\THU_PASS\\Organization\\supervisor-rv\\kernel\\kernel.bin"; // BaseRAM ???????????????????????°§??
  ...
  initial begin
    ...
    // uart.pc_send_byte(8'h80);
    write_asm("D:\\github\\THU_PASS\\Organization\\supervisor-rv\\kernel\\base_test_new.bin",
              32'h8010_0000);
    #300000;
