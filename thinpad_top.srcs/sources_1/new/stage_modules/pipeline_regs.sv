...
module if1_if2 (
    ...
    input  wire stall,
    input  wire bubble,
    // Bubble means bubble, no fetching is needed.
    output reg  if2_bubble,

    input  wire [31:0] bubble_pc,
    input  wire [31:0] if1_vpc,
    output reg  [31:0] if2_vpc,

    input  wire [31:0] if1_ppc,
    output reg  [31:0] if2_ppc,

    input  wire [31:0] if1_pred_pc,
    output reg  [31:0] if2_pred_pc
);
  always_ff @(posedge clock) begin
    if (reset) begin
      ...
      if2_ppc <= `START_ADDR;
      if2_bubble <= 0;
      if2_pred_pc <= (`START_ADDR) + 4;
    end else begin
      if (bubble) begin
        if2_vpc <= bubble_pc;
        if2_ppc <= bubble_pc;
        if2_bubble <= 0;
        if2_pred_pc <= bubble_pc + 4;
